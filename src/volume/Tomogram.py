...
class Tomogram(VolumePlus):
    ...
    def __init__(
        self, session: Session, data: GridData, region=None, rendering_options=None
    ):
        VolumePlus.__init__(
            self, session, data, region=region, rendering_options=rendering_options
        )
        ...
        self.set_parameters(
            image_levels=self.default_levels, color_mode="l8", colormap_on_gpu=True
    ...
    def normal(self, value):
        ...
        self.set_parameters(
            tilted_slab_axis=tuple(value), color_mode="l8"
        )  # backing_color=(0, 0, 0, 255),
    ...
    def create_filtered_tomogram(
        ...
        if lp_method not in ["gaussian", "cosine"] or hp_method not in [
            "gaussian",
            "cosine",
        ]:
        ...
        if unit == "angstrom" and px == (1, 1, 1):
            self.session.logger.warning(
                "Are you sure you're pixelsize is {}?".format(px)
            )

        Nz, Ny, Nx = shape[2], shape[1], shape[0]
        if unit == "pixels":
            zz, yy, xx = np.meshgrid(
                fft.fftfreq(Nz), fft.fftfreq(Ny), fft.rfftfreq(Nx), indexing="ij"
            )
            ...
            zz, yy, xx = np.meshgrid(
                fft.fftfreq(Nz, px[2]),
                fft.fftfreq(Ny, px[1]),
                fft.rfftfreq(Nx, px[0]),
                indexing="ij",
            )
            if (use_lp and lp == 0) or (use_hp and hp == 0):
                self.session.logger.warning(
                    "Cannot have a pass length of 0 angstrom. Use the checkboxes to disable the filter you do not want."
                )
                return
            if use_lp:
                lp = 1 / lp
            ...
            raise NotImplementedError(
                'Only "pixels" and "angstrom" implemented as units.'
            )
        ...
        lp_filt = (
            create_filter(True, r, lp, lpd, method=lp_method, thresh=thresh)
            if use_lp
            else np.ones(r.shape)
        )
        hp_filt = (
            create_filter(False, r, hp, hpd, method=hp_method, thresh=thresh)
            if use_hp
            else np.ones(r.shape)
        )
        filter = np.multiply(lp_filt, hp_filt)
        filtered_data = np.array(
            fft.irfftn(np.multiply(fft.rfftn(self.data.matrix()), filter)),
            dtype=np.float32,
        )
    ...
    def create_averaged_tomogram(self, axis=(0, 0, 1), num_slabs=10):
        ...
        ts = np.linspace(
            -1, 1 + 0.999 / num_slabs, resolution
        )  #  the +0.999/num_slabs part is to make sure that the translated samples goes from 0 to a _little_ bit less than 2*num_slabs+1
        samples = np.array([axis * t for t in ts])
        samples = (
            samples * num_slabs + num_slabs
        )  # translate the points to matrix indices

        # Go through samples and add 1 to the cell the sample is in
        for point in samples:
            conv_matrix[
                tuple(np.flip(np.floor(point).astype(int)))
            ] += 1  # flip needed because coord (x,y,z) in conv_matrix is accessed as conv_matrix[z][y][x]
    ...
    def create_tomo_from_array(self, array, name):
        ...
        new_tomogram = Tomogram(
            self.session, ArrayGridData(array, name=name, step=self.data.step)
        )
    ...
    def create_processable_tomogram(self, num_averaging_slabs=0):
        ...
        processable_tomogram = ProcessableTomogram(
            self.session, self, num_averaging_slabs=num_averaging_slabs
        )
    ...
    def calc_time_map(self, slice, order=1, num_slabs=10, axis=(0, 0, 1)):
        ...
        for j in range(original_data.shape[1]):
            for k in range(original_data.shape[2]):
                pts = [
                    (slice, j, k) + n * axis for n in range(-num_slabs, num_slabs + 1)
                ]
                pts = np.array(pts)
                pts = [pts[:, 0], pts[:, 1], pts[:, 2]]
                running_average_data[slice][j][k] = np.mean(
                    map_coordinates(original_data, pts, order=order),
                    axis=0,
                    dtype=np.float32,
                )
        t1 = time.time()
    ...
    def calc_time_rgi(self, slice, method="nearest", num_slabs=10, axis=(0, 0, 1)):
        ...
        original_data = self.data.matrix()
        points = (
            np.arange(original_data.shape[0]),
            np.arange(original_data.shape[1]),
            np.arange(original_data.shape[2]),
        )
        ...
        for j in range(original_data.shape[1]):
            for k in range(original_data.shape[2]):
                ...
                running_average_data[slice][j][k] = np.mean(
                    rgi(pts), axis=0, dtype=np.float32
                )
        t1 = time.time()
        return t1 - t0
    ...
    def calc_time_rgi_grid(self, row, method="nearest", num_slabs=10, axis=(0, 0, 1)):
        ...
        pts = (
            np.mgrid[
                row - num_slabs : row + num_slabs + 1,
                0 : original_data.shape[1],
                0 : original_data.shape[2],
            ]
            .reshape(3, -1)
            .T
        )
        interpolated_surrounding_rows = rgi(pts).reshape(
            (num_slabs * 2 + 1, original_data.shape[1], original_data.shape[2])
        )
        running_average_data[row] = np.mean(
            interpolated_surrounding_rows, axis=0, dtype=np.float32
        )
    ...
    def calc_time_map_grid(self, row, order=1, num_slabs=10, axis=(0, 0, 1)):
        ...
        zs, ys, xs = np.meshgrid(
            np.arange(row - num_slabs, row + num_slabs + 1),
            np.arange(original_data.shape[1]),
            np.arange(original_data.shape[2]),
            indexing="ij",
        )
        ...
        t0 = time.time()
        interpolated_surrounding_rows = map_coordinates(
            original_data, pts, order=order
        ).reshape((num_slabs * 2 + 1, original_data.shape[1], original_data.shape[2]))
        running_average_data[row] = np.mean(
            interpolated_surrounding_rows, axis=0, dtype=np.float32
        )
    ...
    def set_slab_running_average(self, num_slabs, axis=(0, 0, 1)):
        ...
        for i in range(shape[0]):
            for j in range(shape[1]):
                for k in range(shape[2]):
                    pts = [
                        (i, j, k) + n * axis for n in range(-num_slabs, num_slabs + 1)
                    ]
                    pts = [
                        pt
                        for pt in pts
                        if (pt >= 0).all() and (pt <= (shape - [1, 1, 1])).all()
                    ]
                    running_average_data[i][j][k] = np.mean(
                        rgi(pts), axis=0, dtype=np.float32
                    )
        ...
        running_average_tomogram = Tomogram(
            self.session,
            ArrayGridData(
                running_average_data,
                name="Running average " + str(num_slabs) + " " + self.data.name,
            ),
        )
    ...
    def _set_slab_offset(self, offset=None):
        ...
        self.set_parameters(
            tilted_slab_axis=tuple(self.normal),
    ...
    @is_clipped.setter
    def is_clipped(self, value: bool):
        if value:
            self._set_clipping()
        self._is_clipped = value

    def update_clip(self, name=None, value=None):
        self._set_clipping()

    def _set_clipping(self, offset=None) -> None:
        ...
        vec = self.normal
        pos = (
            self.normal[0] * offset,
            self.normal[1] * offset,
            self.normal[2] * offset,
        )
        run(
            self.session,
            f"clip front {-artia.clip_thickness/2} position {pos[0]},{pos[1]},{pos[2]} axis {vec[0]},{vec[1]},{vec[2]} coordinateSystem #{self.id_string}",
            log=False,
        )
        run(
            self.session,
            f"clip back {artia.clip_thickness/2} position {pos[0]},{pos[1]},{pos[2]} axis {-vec[0]},{-vec[1]},{-vec[2]} coordinateSystem #{self.id_string}",
            log=False,
        )
